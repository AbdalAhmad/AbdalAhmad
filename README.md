<h1 align="center">Hi 👋, I'm Abdal Ahmad</h1>
<h3 align="center">Flutter & Full Stack Developer | Building Real Apps</h3>

---

### 🚀 About Me
- 📱 I build production-level apps using **Flutter + Firebase + Node.js**
- 🌐 Full Stack Developer — REST APIs, Backend logic, UI/UX
- 🧠 Solved 550+ DSA problems (Java)
- ⚙️ Strong in System Design, OOP, DBMS, OS, Networks
- 🎯 Focus: Shipping real-world apps, not just tutorials

---

### 🔥 What I Build
- 📱 Cross-platform mobile apps (Flutter)
- 🌍 Full stack web apps (Node.js + Express)
- 🔗 API integrations & backend systems
- ⚡ Scalable and clean UI/UX applications

---

### 🌐 Connect with Me
<p align="left">
  <a href="https://www.linkedin.com/in/abdal-ahmad-raza-095688232/" target="_blank">
    <img src="https://skillicons.dev/icons?i=linkedin" />
  </a>
  <a href="https://www.instagram.com/abdal_ahmad_raza/" target="_blank">
    <img src="https://skillicons.dev/icons?i=instagram" />
  </a>
 <a href="https://leetcode.com/u/abdalraza251/" target="_blank">
  <img src="https://cdn.jsdelivr.net/gh/simple-icons/simple-icons/icons/leetcode.svg" height="30" width="30"/>
</a>
</p>

---

### 🛠 Tech Stack
<p>
  <img src="https://skillicons.dev/icons?i=flutter,dart,firebase,nodejs,express,mongodb,mysql,js,html,css,git,github,postman" />
</p>

---

### 📌 Featured Work
- 🎮 Flutter Game App — State management, persistence, gameplay logic  
- 📱 App systems with real-time data (Firebase integration)  
- 🌐 Backend APIs using Node.js & Express  

---

### ⚡ What Makes Me Different
- I **ship complete apps**, not just components  
- I debug and fix real-world issues (state, performance, persistence)  
- I use AI tools to accelerate development, not replace thinking  

---

### 📫 Contact
**Email:** abdalraza251@gmail.com
